...
###### Wait, it sounds like "weights" and "denominator" are the same thing.

It depends on your winsorization preferences. At time of writing, winsorization is currently off by default unless one modifies the code. Once its is fully implemented in a forthcoming update, winsorization (if turned on) will be blanket-applied to your dataset based on the values of w_lower and w_upper or a global set of values if the variable-specific ones are unspecified, and in situations where the variable has a specified numerator and denominator, winsorization is (optionally) applied to those as well with the final averages being weighted by the denominator. The weight column allows you to protect certain variables from your global winsorization settings - for example, if you have a dataset with things like agricultural productivity and proportion of female household members, you might want to put things in like household plot area in the denominator column and the number of household members in the weight column, since it can make more sense to winsorize the former than the latter. If you supply both, the app will preferentially use the denominator column. 

###### Great, but none of that applies to my analysis

Then no harm will come from removing the columns or leaving them blank - the goal of this setup is to create something that can be modified to suit a wide variety of analyses and allow teams to make sense of their particular datasets together. There are potential use cases where you could choose to remove the weight column or the numerator and denominator columns, or all three. (But leaving the numerator column in without also filling in the denominator column might result in some unusual estimates).

###### A final note on reproducibility

If you've made it this far in the guide, and noticing that there are still six more tables to go is making you wonder about the advantages to using PEP over a simpler data visualization utility, this is one of them. In some policy analysis spaces, outlier trimming and standardization is often subject to decisionmaking that may be invisible in the product(s), resulting in preparations of the same raw data that differ, often substantially, across organizations. PEP is designed to help keep those decisions explicit and reproducible (and we are in the process of adding additional utilities and configuration options), so we encourage you to use raw data rather than making these decisions in advance.
...
| *`r colorize("Your criteria (one or more columns)", "darkgreen")`* |  ⬆, ⬇, ⬍, or = ; or text. For instance, efficiency (e.g. measured by final price or quantity exchanged), speed of implementation (slow, medium, fast), effectiveness or probablity of success, political/legal feasibility |
:::
